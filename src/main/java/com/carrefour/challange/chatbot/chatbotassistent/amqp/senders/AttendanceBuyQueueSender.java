...
@Component
public class AttendanceBuyQueueSender {

	@Autowired
	private RabbitTemplate template;
	
	@Autowired
	private Queue attendanceBuyMessageToAttendant;
	
	private ObjectMapper mapper = new ObjectMapper();
	
	public void send(Message message) throws JsonProcessingException {
		String json = mapper.writeValueAsString(message);
		template.convertAndSend(attendanceBuyMessageToAttendant.getName(), json);
	}
}
